...
export async function bundleMDX({
	...
	const outputOpts: OutputOptions = {
		format: "esm", // Output ESM from Rolldown first
		sourcemap: false,
		inlineDynamicImports: true, // Keep it as a single ESM string if possible
	};
	console.log("[bundleMDX] Rolldown Output Options (ESM stage):", outputOpts);
	...
	try {
		...
		console.log("[bundleMDX] Calling bundle.write(outputOpts) for ESM...");
		const { output } = await bundle.write(outputOpts);
		console.log(
			"[bundleMDX] Rolldown ESM write successful. Output (length):",
			output.length,
		);

		if (output.length > 0 && output[0].type === "chunk") {
			esmBundledCode = output[0].code;
			console.log(
				"[bundleMDX] Full ESM Bundled code (pre-Sucrase):",
				esmBundledCode,
			);
		} else {
			console.error(
				"[bundleMDX] No ESM chunk generated by Rolldown or unexpected output type.",
			);
			throw new Error(
				"No ESM chunk generated by Rolldown or unexpected output type",
			);
		}
	} catch (error: unknown) {
	...
	// Transpile ESM to CJS using Sucrase
	let cjsCode = "";
	if (esmBundledCode) {
		try {
			console.log("[bundleMDX] Transforming ESM to CJS with Sucrase...");
			const sucraseResult = sucraseTransform(esmBundledCode, {
				transforms: ["imports", "jsx"], // Ensure JSX is preserved/handled if not already pure JS
				jsxRuntime: "automatic", // Or "classic" if needed, but MDX should handle JSX
				jsxImportSource: jsxConfig?.jsxLib?.package, // Consistent with MDX config
			});
			cjsCode = sucraseResult.code;
			console.log(
				"[bundleMDX] Sucrase CJS transformation successful. CJS code (first 500 chars):",
				cjsCode.substring(0, 500),
			);
		} catch (error) {
			console.error("[bundleMDX] Error during Sucrase transformation:", error);
			errors.push(error instanceof Error ? error : new Error(String(error)));
		}
	}

	const result: BundleMDXResult = {
		code: cjsCode, // This is now the CJS code
		frontmatter,
